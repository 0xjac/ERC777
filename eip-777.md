...
This call MUST:
 ...
 - call the `tokensReceived` method on the address implementing `ITokenRecipient` as returned by a EIP-820 lookup on the `to` address, regardless if `to` is a regular address or a contract.
 - fire the `Sent` event

If `to` is a contract which is not prepared to receive tokens. Specifically, it is a contract which does not register an address (its own or another) via EIP-820 implementing the `ITokenRecipient` interface. Then `send` MUST throw.
...
This call MUST:
 ...
 - call the `tokensReceived` method on the contract implementing `ITokenRecipient` as returned by a EIP-820 lookup on the `to` address
 - fire the `Sent` event

If `to` is a contract which is not prepared to receive tokens. Specifically, it is a contract which does not register an address (its own or another) via EIP-820 implementing the `ITokenRecipient` interface. Then `operatorSend` MUST throw.
...
Minting MUST follow the same rules as `send`/`operatorSend`. I.e. if `to` is a contract which is not prepared to receive tokens. Specifically, it is a contract which does not register an address (its own or another) via EIP-820 implementing the `ITokenRecipient` interface. Then the minting MUST throw.

If the `to` register an address via EIP-820 implementing the `ITokenRecipient` interface, `tokensReceived` MUST be called.
...
Any address (contract or regular account) CAN register a contract (itself or an other) implementing the `ITokenRecipient` interface via the EIP-820 registry.
...
The only difference for new contracts implementing ERC-20 is that registration of `ERC777TokensSender` and `ITokenRecipient` via EIP-820 takes precedence over ERC-20. This means that even with on a ERC-20 `transfer` call, the token contract MUST check via EIP-820 if the `from`/`to` address implements `tokensToSend`/`tokensReceived` and call it if available. Note that when calling ERC-20 `transfer` on a contract, if the contract does not implement `tokensReceived`, the `transfer` call SHOULD still be accepted even if this means the tokens will probably be locked.
