...
3. Contracts and regular addresses can control which tokens are authorized to be received.
...
*NOTE*: By default the token holder (`msg.sender`) MUST be the only one allowed manage its tokens and MUST NOT be forbidden to manage its own token. This method is intended to allow verified contracts like an exchange, a check processor or an automatic charging system to manage tokens on
someone's behalf.

*NOTE*: A token holder CAN authorize multiple operators at the same time.

*NOTE*: If the sender is also the operator. He is free to put data in the `operatorData` parameter if appropriate such as a payment reference which is expected to be provided by an operator.
...
If `to` is a contract and the ERC-672 lookup does not return an address implementing `tokensReceived`, the minting MUST follow the same rules as transfer.
...
Therefore a token contract can implement both ERC20 and ERC777 in parallel. Read-only functions (such as `name`, `symbol`, `balanceOf`, `totalSupply`) and internal data (such as the mapping of balances) overlap without problem. The write methods from both standards are decoupled and can operate independently from each other. Note that ERC20 functions SHOULD be limited to only being called from old contracts.
