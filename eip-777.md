...
If `to` is a contract which is not prepared to receive tokens. Specifically, it is a contract that neither registers an address (its own or another) via EIP-820 implementing the `ITokenRecipient` interface nor whose hash of the source code is in the whitelisted codes referenced in the appendix of this standard. Then `send` MUST throw.
...
If `to` is a contract which is not prepared to receive tokens. Specifically, it is a contract that neither registers an address (its own or another) via EIP-820 implementing the `ITokenRecipient` interface nor whose hash of the source code is in the whitelisted codes referenced in the appendix of this standard. Then `operatorSend` MUST throw.
...
Minting MUST follow the same rules as `send`/`operatorSend`.
I.e. if `to` is a contract which is not prepared to receive tokens. Specifically, it is a contract that neither registers an address (its own or another) via EIP-820 implementing the `ITokenRecipient` interface nor whose hash of the source code is in the whitelisted codes referenced in the appendix of this standard. Then the minting MUST throw.
