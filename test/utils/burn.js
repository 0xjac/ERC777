  ...
  describe('burn', function() {
    ...
    it(`should let ${utils.formatAccount(accounts[0])} burn 3 ${token.symbol}`,
      async function() {
        ...
        await utils.assertBalance(web3, token, accounts[0], token.initialSupply + 7);
        await utils.assertTotalSupply(web3, token, 10 * accounts.length + token.initialSupply - 3);
        await eventsCalled;
      }
    );
